...
const CrimeReportForm = () => {
  ...
  useEffect(() => {
    if (selectedDivision) {
      ...
      axios
        .get(
          `https://bdapis.com/api/v1.2/districts?division=${selectedDivision}`
        )
        .then((response) => {
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div>
          ...
          <input
            type="datetime-local"
            {...register("crimeTime", {
              required: "Please select the time of the crime",
            })}
            onChange={(e) => setCrimeTime(e.target.value)}
            className="input input-bordered w-full"
          />
          {errors.crimeTime && (
            <p className="text-red-500 text-sm">{errors.crimeTime.message}</p>
          )}
        </div>
        ...
        <div>
          <button
            type="submit"
            className="px-2 py-2 bg-purple-500  rounded-md text-white cursor-pointer w-full"
